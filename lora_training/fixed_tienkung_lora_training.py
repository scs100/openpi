...
class SGDWithClipping(_optimizer.OptimizerConfig):
    """SGD optimizer with gradient clipping support."""

    momentum: float = 0.9
    nesterov: bool = False
    clip_gradient_norm: float = 2.0
    ...
    def create(
        ...
        # 创建SGD优化器
        sgd_tx = optax.sgd(lr, momentum=self.momentum, nesterov=self.nesterov)

        # 添加梯度裁剪
        return optax.chain(optax.clip_by_global_norm(self.clip_gradient_norm), sgd_tx)
...
def setup_logging():
    ...
    logger, log_file = setup_test_logger(f"{optimizer_name}_lora_{EXPERIMENT_NAME}", "logs")
...
def create_dynamic_training_config(initial_batch_size):
    ...
    config = _config.TrainConfig(
        ...
        ) if USE_ADAMW else SGDWithClipping(
            momentum=SGD_MOMENTUM,
            nesterov=SGD_NESTEROV,
            clip_gradient_norm=SGD_CLIP_NORM,  # 使用自定义SGD支持梯度裁剪
        ),
